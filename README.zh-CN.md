...
输入提示保持简短（`You >`），底部两行状态栏固定显示当前聊天、
空闲/运行/待审批状态、忙碌时间和排队数量，并提示正在运行的工具或常用命令。
长标题会根据窗口宽度截断。工具运行过程不再刷屏，结束或失败时才输出一行摘要；
提交的输入也不会重复回显。

Agent 回复支持 **Markdown 显示**：标题、粗体/斜体、列表、引用、行内代码、
语法高亮代码块和表格。流式回复按完整块显示一次；未结束的段落、列表或代码块
会等到边界明确或回复结束后显示，期间底栏提示正在接收。
表格长单元格会折行，不会悄悄省略文字。链接以文字显示，图片显示占位符，
不会自动打开或下载；审批详情仍按原文展示。
如果更喜欢无颜色界面，可以在启动前设置 `NO_COLOR=1`。

已安装过终端模式的用户，请重新执行上面的可选依赖安装命令以补齐 Markdown 依赖。
窗口过窄或嵌套过深时，会明确提示并显示原文；未完成块超过 64 Ki 字符时，
该回复剩余部分改为纯文本，避免无限缓存。
