...
使用 Microsoft 时，电脑启动参数改为 `--devtunnel-login microsoft`，
手机也选择 Microsoft 登录。之后启动时可省略登录参数，复用电脑 CLI 已有的账号。

**预览功能限制：** 两种账号方式均尚未完成真实手机上的登录、发现、配对全流程验证。
已配置的 Microsoft 应用可以申请设备登录代码，但用户授权和 Dev Tunnels 访问权限
仍需实际验证。账号发现不可用时，请使用二维码配对。
...
Microsoft 登录可以刷新其支持的凭据；GitHub 凭据过期或被撤销后需要重新登录。
