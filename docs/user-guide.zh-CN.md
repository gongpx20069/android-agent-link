...
[快速开始](../README.zh-CN.md) · [English](user-guide.md) | [中文](user-guide.zh-CN.md)
...
按住鼠标左键拖动即可选择会话中的部分文字，无需按 Shift；松开后按 **Ctrl+Y**
复制高亮片段。拖选不会误触工具展开/折叠。右侧滚动条支持点击和拖动，可浏览
终端已保留的会话内容；按 `End` 回到最新消息。
向上浏览时不会被新消息拉回底部。
点击输入框会恢复输入和自动跟随；焦点在会话区时直接打字或粘贴，也会回到输入框，
不会丢掉文字。`Esc`、`Tab` 返回输入区，或发送消息时，都会恢复显示最新内容。
仅用 `Tab` 切入会话区不会暂停跟随，主动滚动、定位或展开工具时才暂停。
...
不会影响 Android 接收完整事件。
CLI 输入 `/copy` 复制最新回复的已保留原文（含 Markdown）；用 `Tab` 和方向键定位
消息或工具后，没有文字选区时按 `Ctrl+Y` 复制该条内容，包含缓存中的其他页。
存在鼠标选区时只复制高亮的显示文字，不包含隐藏详情或其他源码页。内容被截断或淘汰时明确提示。
复制目标是运行 bridge 的电脑剪贴板，不是手机或 SSH 客户端；失败会提示，不会假装成功。
`/mouse` 可关闭应用鼠标接管，使用终端原生拖选和复制，再执行一次恢复工具点击。
`Ctrl+C` 仍用于清空输入，不承担复制。

Android 长按消息文字后，拖动选区两端选择需要的片段，再点击系统菜单中的「复制」。
计划步骤和展开的工具详情也支持选字，不再显示「复制全文」按钮。选区限定在当前页，
其他内容可翻页后选择。代码片段仍保留复制按钮；超过 128 Ki UTF-16 代码单元时明确
拒绝整段代码复制且不覆盖剪贴板，不会悄悄截断。
...
| `/use 2` | 直接切换到编号 2 的聊天。 |
| `/tools` | 定位最新的可折叠工具组。 |
| `/copy` | 复制最新回复的已保留原文到电脑剪贴板。 |
| `/mouse` | 切换应用鼠标接管，以使用终端原生文字选择。 |
...
可以在 Android 创建或打开会话，也可以在终端 `/new` 后刷新 Android 的共享聊天列表。
bridge 会保存共享目录和有界事件记录；终端选择及实时显示不会跨 bridge 重启恢复。
CLI 交互模式启动时不再打印二维码/链接，需要时输入 `/qrcode` 调出独立页面，
不会被新消息冲掉；普通服务模式仍在启动时显示。二维码仍使用启动时的短期令牌，
查看不会延长有效期，过期后需重启 bridge。
...
在同一台手机上从 Mochi 的工具设置连接 AgentLink，跳转后确认调用方、资源和能力范围。
电脑凭据仍由 AgentLink 保管，执行审批仍由用户决定。
...
| 两台电脑只发现一台 | 更新 Android App：全局列表缺少 bridge 端口时会补查所在区域的详情。两台电脑都需运行 bridge，并使用不同隧道 ID，不要都指定 `--devtunnel-id agentlink`。 |
| Dev Tunnels 提示 `Login token expired` | 执行 bridge 错误提示中的完整 CLI 登录命令，再重启；手机登录不会刷新电脑端登录。 |
| `ClientSSH ... send window is full` | Dev Tunnels 正等待 SSH 通道接收窗口更新，偶发时通常可自行恢复；若持续刷出且输出停滞，应检查网络以及接收端 App 是否在正常消费数据。单凭这条提示不能判断 agent 执行失败。 |
...
如需构建或扩展 AgentLink，请阅读[技术文档](README.md)和
[贡献者说明](../CLAUDE.md)。
