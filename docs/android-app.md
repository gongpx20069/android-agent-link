...
`resume` is a built-in AgentLink chip rather than a prompt command. It opens a session picker backed by ACP `session/list`; choosing a session calls `session/load` for the current chat and workspace. Copilot ACP replays recent loaded history newest-first, so AgentLink takes the first N loaded chat message bubbles and displays that window oldest-to-newest. N is configured in Settings and defaults to 5. Tool/activity cards and hidden command/config control updates do not count toward N. Typing `/resume` in the prompt is still treated as plain prompt text unless the ACP agent explicitly advertises a `resume` slash command.
